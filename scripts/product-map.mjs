...
/**
 * At most BULLET_LIMIT characters, ellipsis included. A cut that lands inside a code span drops
 * that partial span instead of closing it: an auto-closed span would read as a complete path or
 * identifier the changelog never wrote.
 */
const truncate = (text) => {
  if (text.length <= BULLET_LIMIT) return text;
  let cut = text.slice(0, BULLET_LIMIT - 1);
...
/** A pipe in a cell ends the row in GFM, code span or not, so it has to be escaped. */
const cell = (text) => String(text).replace(/\|/g, "\\|");

export function renderGenerated({ endpoints, tables, adrs, changelog }) {
  const out = [];
  out.push(
  ...
  for (const row of endpoints) {
    out.push(
      `| ${cell(row.tag)} | ${cell(row.method)} | ${code(cell(row.path))} | ${cell(row.summary)} |`,
    );
  }
