...
describe("POST /tasks", () => {
  ...
  it("returns 201 with aiStatus failed when the queue is unavailable", async () => {
    ...
    const task = await createTask(ctx.server, user.token, { title: "Queue is down today" });
  ...
  it("creates a child under a root with aiStatus skipped and no job; a grandchild fails with the depth message", async () => {
    const user = await registerUser(ctx.server);
    const root = await createTask(ctx.server, user.token, { title: "Root task here" });
    ctx.queue.jobs.length = 0;
    const child = await createTask(ctx.server, user.token, {
      title: "First step",
      parentId: root.id,
    });
    ...
    const grandchild = await request(ctx.server)
      .post(TASKS)
      .set(auth(user.token))
  ...
  it("assigns child positions after the current maximum", async () => {
    const user = await registerUser(ctx.server);
    const root = await createTask(ctx.server, user.token, { title: "Ordered steps" });
    const a = await createTask(ctx.server, user.token, { title: "A", parentId: root.id });
    const b = await createTask(ctx.server, user.token, { title: "B", parentId: root.id });
  ...
  it("rejects a foreign parent with NOT_FOUND and foreign tag ids with VALIDATION_ERROR", async () => {
    ...
    const root = await createTask(ctx.server, owner.token, { title: "Owner root" });
    const foreignParent = await request(ctx.server)
      .post(TASKS)
    ...
    const foreignTag = await request(ctx.server)
      .post(TASKS)
      .set(auth(other.token))
  ...
  it("with AI_ENABLED=false creates the task skipped with reason ai_disabled", async () => {
    ...
    try {
      ...
      const task = await createTask(paused.server, user.token, { title: "Paused assistant" });
  ...
  it("past the user limit still creates the task, skipped with reason rate_limited", async () => {
    ...
    try {
      ...
      const first = await createTask(limited.server, user.token, { title: "First one counts" });
      const second = await createTask(limited.server, user.token, { title: "Second is limited" });
...
describe("GET /tasks/:id", () => {
  it("returns children, tags and progress computed over user and accepted children", async () => {
    const user = await registerUser(ctx.server);
    const root = await createTask(ctx.server, user.token, { title: "Root with children" });
    const c1 = await createTask(ctx.server, user.token, { title: "Child one", parentId: root.id });
    await createTask(ctx.server, user.token, { title: "Child two", parentId: root.id });
    await ctx.db.update(tasks).set({ status: "done" }).where(eq(tasks.id, c1.id));
    const res = await request(ctx.server).get(`${TASKS}/${root.id}`).set(auth(user.token));
  ...
  it("returns NOT_FOUND for another user's task and for an unknown id", async () => {
    const owner = await registerUser(ctx.server);
    const other = await registerUser(ctx.server);
    const root = await createTask(ctx.server, owner.token, { title: "Private task" });
    expect(
      (await request(ctx.server).get(`${TASKS}/${root.id}`).set(auth(other.token))).status,
    ).toBe(404);
    expect(
      (
    ...
    expect(
      (await request(ctx.server).get(`${TASKS}/not-a-uuid`).set(auth(owner.token))).status,
    ).toBe(400);
  });
});
...
describe("GET /tasks", () => {
  it("walks three pages without duplicates, newest first, nextCursor null on the last page", async () => {
    ...
    for (let i = 1; i <= 7; i += 1) {
      created.push((await createTask(ctx.server, user.token, { title: `Task number ${i}` })).id);
    }
    const seen: string[] = [];
    ...
    do {
      const url: string = cursor
        ...
        : `${TASKS}?limit=3`;
      const res: request.Response = await request(ctx.server).get(url).set(auth(user.token));
      expect(res.status).toBe(200);
  ...
  it("filters by status and by tag, and lists top-level only unless parentId is given", async () => {
    ...
    const tagged = await createTask(ctx.server, user.token, {
      title: "Tagged root",
      tagIds: [tag.body.data.id],
    });
    const plain = await createTask(ctx.server, user.token, { title: "Plain root" });
    const child = await createTask(ctx.server, user.token, {
      title: "A child",
      parentId: plain.id,
    });
  ...
  it("rejects a garbage cursor and an out-of-range limit", async () => {
    const user = await registerUser(ctx.server);
    const bad = await request(ctx.server).get(`${TASKS}?cursor=%%%`).set(auth(user.token));
    expect(bad.status).toBe(400);
    expect(bad.body.error.details).toEqual([{ path: "query.cursor", message: "Invalid cursor" }]);
    const big = await request(ctx.server).get(`${TASKS}?limit=500`).set(auth(user.token));
...
describe("DELETE /tasks/:id", () => {
  it("cascades to children and tag links, and hides other users' tasks", async () => {
    ...
    const root = await createTask(ctx.server, owner.token, {
      title: "Doomed root",
      tagIds: [tag.body.data.id],
    });
    ...
    expect(
      (await request(ctx.server).delete(`${TASKS}/${root.id}`).set(auth(other.token))).status,
    ).toBe(404);
    expect(
      (await request(ctx.server).delete(`${TASKS}/${root.id}`).set(auth(owner.token))).status,
    ).toBe(204);
    expect(
      (await request(ctx.server).get(`${TASKS}/${child.id}`).set(auth(owner.token))).status,
    ).toBe(404);
